 ...
 */
@WebServlet("/FindEditOrder")
public class FindEditOrder extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FindEditOrder() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
         ...
         Float discount=Float.parseFloat("1");
          ...
          try {
			...
			Float lastFee=listModel.get(0).getExpectCost()-(listModel.get(0).getDays()*listModel.get(0).getPrice()*discount);
			if(listModel.size()>0){
				request.setAttribute("zhehoujia", zhehoujia);
