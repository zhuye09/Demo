...
<form id="tab" method="post" action="${pageContext.request.contextPath }/AdminEditRoom">
<div class="row">
  <div class="col-md-4">
    <br>
    <div id="myTabContent" class="tab-content">
      ...
      <c:forEach items="${requestScope.items }" var="item" varStatus="vs">
        ...
        <c:forEach items="${requestScope.roomType}" var="roomType" varStatus="vs">
	       <option value="${roomType.id}"<c:if test="${roomType.id==item.typeId}">selected</c:if>>${roomType.typeName}</option>
        </c:forEach>
	    </select>
        </div>
        <div class="form-group">
    	...
    	<c:forEach items="${requestScope.items}" var="rooms" varStatus="vs">
    		<!-- <c:if test="${rooms.positions}">selected</c:if> -->
           	<option value="1F" <c:if test="${rooms.positions.equals('1F')}">selected</c:if>>1F</option>
           	<option value="2F" <c:if test="${rooms.positions.equals('2F')}">selected</c:if>>2F</option>
           	<option value="3F" <c:if test="${rooms.positions.equals('3F')}">selected</c:if>>3F</option>
           	<option value="4F" <c:if test="${rooms.positions.equals('4F')}">selected</c:if>>4F</option>
           	<option value="5F" <c:if test="${rooms.positions.equals('5F')}">selected</c:if>>5F</option>
           	<option value="6F" <c:if test="${rooms.positions.equals('6F')}">selected</c:if>>6F</option>
        </c:forEach>
    	
    </select>
        
        <input type="hidden" name="state" value="${item.state}" class="form-control"  >
        </div>
        </c:forEach>
      </div>
    </div>
