...
<li><a href="${pageContext.request.contextPath}/AdminSuperMen"><span class="fa fa-caret-right"></span>高级用户</a></li>
    ...
    <div class="content">
        <div class="header">
            ...
            <li><a href="AdminFindUser">用户列表</a> </li>
        </ul>
        </div>
        <div class="main-content">
<div class="btn-toolbar list-toolbar">
    <li class="btn btn-primary"><a class="adminadd" href="${pageContext.request.contextPath}/ViewRoleServlet">添加用户</a></li>
  <div class="btn-group">
  </div>
</div>
<table class="table">
...
<c:forEach items="${requestScope.items.list }" var="item" varStatus="vs">
	  <tr>
	    <td >${(items.pageIndex-1) * items.pageSize + vs.count}</td>
	    <td >${item.userName }</td>
	    <td >${item.trueName}</td>
	    <td >${item.tel }</td>
	    <td >${item.roleName }</td>
	    <td ><c:if test="${item.userName!='admin' && item.state == 0  }"> 
	    <a href="${pageContext.request.contextPath}/AdminOnState?id=${item.id}" >启用</a> </c:if>
      <c:if test="${item.userName!='admin' && item.state == 1  }">
       <a href="${pageContext.request.contextPath}/AdminOffState?id=${item.id}" onClick="javascript:return confirm('确认禁用该用户？')">禁用</a> </c:if></td>
	    <td>
	    <c:if test="${item.userName!='admin'}">
          <a href="${pageContext.request.contextPath}/AdminFindById?id=${item.id}"><i class="fa fa-pencil"></i></a>
        </c:if>
        <c:if test="${item.userName!='admin'}">
          <a href="${pageContext.request.contextPath}/DelUser?id=${item.id }" onclick="sureDel()" role="button" data-toggle="modal">删除</a>
        </c:if>
        </td>
	  </tr>
</c:forEach>
...
<p:page pageIndex="${items.pageIndex }" pageSize="${items.pageSize }" actionName="${pageContext.request.contextPath }/AdminFindUser" pageNums="${items.pageNums }" hasPreOmit="${items.hasPreOmit }" recordCount="${items.recordCount }" hasSuffixOmit="${items.hasSuffixOmit }" where="${items.where }"></p:page>
     	<footer>
	          <hr>
