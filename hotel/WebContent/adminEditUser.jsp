...
<li><a href="${pageContext.request.contextPath}/AdminSuperMen"><span class="fa fa-caret-right"></span>高级用户</a></li>
    ...
    <div class="content">
        <div class="header">
            ...
            <li><a href="users.jsp">用户资料</a> </li>
        </ul>
        </div>
        <div class="main-content">
<ul class="nav nav-tabs">
  <li class="active"><a href="#home" data-toggle="tab">详细资料</a></li>
</ul>
 <form id="tab" method="post" action="${pageContext.request.contextPath}/AdminEditUser">
<div class="row">
  <div class="col-md-4">
    <div id="myTabContent" class="tab-content">
      ...
      <c:forEach items="${requestScope.items}" var="item" varStatus="vs">
        ...
        <input type="text" value="${item.userName}" name="userName" readonly="readonly" class="form-control">
        </div>
        <div class="form-group">
        <label>真实姓名</label>
        <input type="text" value="${item.trueName}" name="trueName"  class="form-control">
        </div>
        <div class="form-group">
        <label>性别</label>
       <c:if test="${item.gender.equals('男')}">
         <input type="radio" name="gender" id="radio1" value="男" checked="checked">男
         <input type="radio" name="gender" id="radio2" value="女" >女
        </c:if>
       <c:if test="${item.gender.equals('女')}">
        ...
        <div class="form-group">
        <label>电话</label>
        <input type="text" value="${item.tel}" name="tel" class="form-control">
        </div>
        <div class="form-group">
        <label>入职时间</label>
        <input type="text" value="${item.enrollDate}" onClick="new WdatePicker()"  name="enrollDate" class="form-control">
        </div>
        <div class="form-group">
        <label>角色</label>
         <select   name="roleId" id="roleId" class="form-control">
                <c:forEach items="${requestScope.role}" var="role" varStatus="vs">
                 <option value="${role.id }" 
                  <c:if test="${role.id==item.role_id}">selected</c:if> 
