...
public class OrderListDao extends BaseDao {
	...
	public int findAllOrderListCount(String where) throws Exception {
		...
		String sql = " SELECT COUNT(*) FROM customer_orderlist AS ol LEFT JOIN customer AS c ON c.id=ol.customer_id LEFT JOIN orderlist AS o ON ol.orderlist_id=o.id LEFT JOIN rooms AS r ON c.roomid=r.id LEFT JOIN roomtype AS rt ON r.typeId=rt.id  WHERE  " + where;
		return (Integer) helper.executeScalar(sql);
